...
class BigramLanguageModel:
    ...
    def compute_bigram_prob_laplace(self, alpha):
        # TODO: Implement here - Q5
        self.bi_prob = self.bi_counts.copy()
        for x in range(0, self.n):
            # formula provided on assignment sheet
            self.bi_prob[x] = ((alpha+self.bi_prob[x])/((alpha*self.n)+np.sum(self.bi_counts[x])))
    ...
    def compute_bigram_prob_interpolation(self, beta, alpha):
        # TODO: Implement here - Q7
        laplace = np.zeros(self.n)
        self.compute_bigram_prob_laplace(alpha)
        for x in range(self.n):
            laplace[x]=(alpha+np.sum(self.bi_counts[x]))/(self.n*alpha+np.sum(self.bi_counts))
        for i in range(len(self.bi_prob)):
            self.bi_prob[i]=beta*self.bi_prob[i]+((1-beta)*laplace)
        # Do not remove the following normalization check.
        for i in range(self.bi_prob.shape[0]):
            assert abs(1 - np.sum(self.bi_prob[i])) < 1e-5
    ...
    def test(self, corpus):
        ...
        for i, idx in enumerate(corpus[:-1]):
            # added this following if statement for situations where divide by zero is encountered in log
            if self.bi_prob[idx, corpus[i+1]] is not 0:
...
class Tokenizer:
    ...
    #  Question 2 - Exploring four tokenization types.
    ...
    # Question 1 - Implement count ngrams in Tokenizer.
...
def test_ngram_counts(tokenizer):
    ...
    # # Testing ngram counts
    # print(test_ngram_counts[0][('the',)])
    # print(test_ngram_counts[0][('saw',)])
    # print(test_ngram_counts[0][('dog',)])
